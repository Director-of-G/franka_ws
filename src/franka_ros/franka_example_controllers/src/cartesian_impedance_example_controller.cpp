...
void CartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  if (counter % 200 == 0) {
    ...
    std::cout << "force estimation error: " << (tau_ext - jacobian.transpose() * F_ext) << std::endl;
    std::cout << "task error: " << error << std::endl;
    std::cout << "closed-loop error: " << - jacobian.transpose() * cartesian_stiffness_ * error + tau_nullspace + tau_ext << std::endl;
  }
  counter++;
