...
void HapticDevice::PublishHapticData()
{
    ...
    // pos.vector.x = position_[0]-initial_position[0];
    // pos.vector.y = position_[1]-initial_position[1];
    // pos.vector.z = position_[2]-initial_position[2];
    geometry_msgs::Point pos;
    pos.x = position_[0]-initial_position[0];
    pos.y = position_[1]-initial_position[1];
    pos.z = position_[2]-initial_position[2];
...
void HapticDevice::RegisterCallback()
{
    ...
    position_pub_ = nh_.advertise<geometry_msgs::Point>(position_topic_.c_str(),1);
...
{   // get and we will publish immediately
    ...
    while (ros::ok() && (keep_alive_ == true)) {
        ...
        if (device_count_ >= 1 && dev_id_ >= 0) {
                ...
                ROS_INFO("position is: %f ,%f ,%f", position_[0]-initial_position[0], position_[1]-initial_position[1], position_[2]-initial_position[2]);
