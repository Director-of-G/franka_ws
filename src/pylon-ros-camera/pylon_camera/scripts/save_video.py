...
class ImageShow(object):
    def __init__(self):
        ...
        # self.pos = []
        self.output = cv2.VideoWriter(self.current_path+"/Videos/record_"+self.name+str(time_tuple[1])+"_"+str(time_tuple[2])+"_"+str(time_tuple[3])+"_"+str(time_tuple[4])+".mp4",self.codec, self.fps, self.frameSize)
        self.circle_list = []
        self.bridge = CvBridge()
        rospy.Subscriber('/aruco_simple/result',Image,self.callback)
        rospy.Subscriber('/aruco_simple/pixel2',PointStamped,self.callback2)
    ...
    def callback(self,data):
        ...
        if self.name =="point":
            cv2.circle(self.img, self.pos, 40,(0,0,255),-1)
